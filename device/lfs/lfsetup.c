	...
	)
{
	...
	if (ibnum == LF_INULL) {
		...
		if (ibnum == LF_INULL) { /* empty file - get new i-block*/
            ...
            ibptr -> isDirectory = 0;
            int i = 0;
            for (i = 0; i < sizeof(struct ldentry); i ++) {
                lfptr -> parent -> lf_unused [i] = '\0';
            }
            memcpy(lfptr -> parent -> lf_unused, (char *)ldptr, sizeof(struct ldentry));
            write (Lf_data.lf_dskdev, (char *)(lfptr -> parent), lfptr -> parentd);
